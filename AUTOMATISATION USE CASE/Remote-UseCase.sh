...
#cmd_send_script="scp -i $NOM_CLE_DEPLOY -o StrictHostKeyChecking=no $NOM_SCRIPT ubuntu@$bastion_Ip:~/"
#echo "cmd_send_script ...: $cmd_send_script"

#------------------ Send Host Script to bastion

scp -i $NOM_CLE_DEPLOY -o StrictHostKeyChecking=no $NOM_SCRIPT_HOSTS ubuntu@$bastion_Ip:~/
...
scp -i $NOM_CLE_DEPLOY -o StrictHostKeyChecking=no $NOM_KEY_INTERCONNECTION ubuntu@$bastion_Ip:~/
...
#cmd_send_conso="scp -i $NOM_CLE_DEPLOY -o StrictHostKeyChecking=no $NOM_JOB_CONSUMER ubuntu@$bastion_Ip:~/"
scp -i $NOM_CLE_DEPLOY -o StrictHostKeyChecking=no $NOM_JOB_CONSUMER ubuntu@$bastion_Ip:~/
...
ssh -i $NOM_CLE_DEPLOY ubuntu@$bastion_Ip sh $NOM_SCRIPT_BASTION
echo "Lunching use case ..."
#cmd_lunch_bastion="ssh -i $NOM_CLE_DEPLOY ubuntu@$bastion_Ip sh $NOM_SCRIPT_BASTION"
